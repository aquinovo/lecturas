...
<html lang="en">
  <head>
   ...
   <script type="text/javascript">
      ...
      function myFunction() {
        setInterval(function(){ <?php include('lib/config.php');  ?>  }, 10000*3600);
      }
    </script>
   ...
   <body>
    <div class="page-wrap">
      <?php
        ...
        setTimeout(function(){
        location.reload()
        },10000 // tiempo en milisegundos ( 10s)
        );
      ?>	
     ...
     <h2><b>NOTA: Después de enviar el reporte de lectura, checar si esta agregado en el kardex, de lo contrario, volver a enviarlo.</b></h2>
      ...
      <section class="doscolumnas-izq"> 
		<div class="redactar">
		  <?php  
             ...
             $consulta = "SELECT matricula,correoE,carrera_nombre,semestre,grupo_nombre FROM alumno WHERE id = '".$_SESSION['uid']."'";
            ...
            if(isset($_POST['enviar'])) {
                ...
                if( !empty($_POST['texto']) && !empty($_POST['caracteres']) && $_POST['caracteres']>=7500 && $_POST['caracteres']<=10844){
                  ...
                  if($datos1 = mysqli_fetch_assoc($resultado1)){
                    ...
                    $string = str_replace(array("'"),'',$resumen);
                    $consulta = "SELECT id, alumno_id,lectura FROM revision_usuario WHERE alumno_id = '".$_SESSION['uid']."' and lectura='".$lectura."'"; 
                    $resultado = ConsultaBD($consulta,$conexion); 
                    $datos = mysqli_fetch_array( $resultado ); 
                    if( $datos ){
